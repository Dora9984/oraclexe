...
SELECT UPPER('higgins') FROM dual; -- dual은 임시테이블. 단순 결과만 보고싶을 때

-- INITCAP() 함수 - 문자열의 첫 글자를 대문자로 변환 -- 되도록 값에 함수를 쓰는것이 나음
SELECT employee_id, last_name, department_id
FROM employees
...
SELECT SUBSTR('HelloWorld', 1, 5) FROM dual; -- 1번째~5번째 문자열 출력
...
SELECT TRIM(' ' FROM ' HelloWorld ') FROM dual; -- 공백 제거할때 많이 사용

-- 숫자 함수
-- ROUND() - 지정된 소수점 자릿수로 값을 반올림합니다. 
SELECT ROUND(45.926, 2) FROM dual;

-- TRUNC() - 지정된 소수점 자릿수로 값을 절삭합니다. 
...
*/
SELECT MONTHS_BETWEEN(TO_DATE('2017-05-22', 'YYYY-MM-DD'), TO_DATE('2016-12-22', 'YYYY-MM-DD')) FROM dual;
...
SELECT last_name, UPPER(CONCAT(SUBSTR(last_name, 1, 8), '_US')) -- 가장 안쪽의 함수부터 실행
FROM employees
