...
AS SELECT employee_id AS 아이디, last_name 이름, salary*12 연봉
    FROM employees 
    WHERE department_id = 50;
    
SELECT * FROM salvu_hangle;

-- 뷰 수정 : 뷰에서 수정하면 실제 테이블도 수정된다. 
...
WHERE employee_number = 200; -- READ ONLY라서 실행 안됨 

/*
WITH CHECK OPTION
    뷰에 대한 데이터 변경작업 제한
*/
-- salary > 10000인 사원에 대해서만 변경 가능
...
-- WITH CHECK OPTION에 위배됩니다. 
UPDATE high_salary_employee_vu SET
salary = 2400
WHERE employee_id = 100;

-- WITH CHECK OPTION에 위배되지 않아 변경 가능합니다.
UPDATE high_salary_employee_vu SET
salary = 24000
WHERE employee_id = 100;
