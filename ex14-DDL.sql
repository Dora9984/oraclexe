...
-- departmens 테이블을 dept에 데이터 복사하기
INSERT INTO dept
SELECT department_id, department_name, location_id, SYSDATE
FROM departments;

/*
CATS는 "Create Table As Select"의 약자로
...
/*
제약조건(Constraints)
    테이블의 해당 컬럼에 원하지 않는 데이터를 입력/수정/삭제 되는 것을 방지하기 위해
    테이블 생성 또는 변경시 설정하는 조건입니다. (저장된 데이터의 신뢰성 높이기 위함)
    
    NOT NULL
        NULL 로 입력이 되어서는 안되는 컬럼에 부여하는 조건으로
...
CREATE TABLE dept4(
    deptno NUMBER(2) CONSTRAINT dept4_dpetno_pk PRIMARY KEY,
...
INSERT INTO dept5 (deptno, dename) VALUES (1, '개발부');
INSERT INTO dept5 (deptno, dename) VALUES (2, '기획부');
INSERT INTO dept5 (deptno, dename) VALUES (3, '디자인');
