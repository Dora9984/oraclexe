...
JOIN
    SQL에서 두 개 이 상의 테이블에서 관련된 행들을 결합하기 위한 연산입니다. 
*/
...
SELECT department_id, department_name,
        location_id, city
FROM departments
NATURAL JOIN locations; -- departments 테이블과 locations 테이블의 location_id가 같으니까 자동으로 테이블을 결합해줌
...
-- ON 절 사용하여 3-Way 조인 -- 같이 들어있는 칼럼을 이용해서 JOIN
...
FROM employees e JOIN departments d
ON e.department_id = d.department_id
AND e.manager_id = 149; -- WHERE 에 써도 됨 
...
INSERT INTO job_grades VALUES ('E', 15000, 2999);
...
INNER JOIN과 OUTER JOIN
    ...
    OUTER JOIN
        한 테이블 행을 기반으로 다른 테이블과의 연결이 없는 행까지 포함하여 반환합니다.
...
FROM employees e
LEFT OUTER JOIN departments d -- JOIN 기준 왼쪽 테이블(employees)은 모두 출력되도록
ON e.department_id = d.department_id;
...
/*
Cartesian Product
    JOIN 조건이 잘못되거나 완전히 생략된 경우 결과가 모든 행의 조합이 표시되는 
